...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, opt=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if not self.loaded_iter:
            with open(scene_info.ply_path, 'rb') as src_file, open(os.path.join(self.model_path, "input.ply") , 'wb') as dest_file:
                dest_file.write(src_file.read()) #data에 있는 ply를 output 폴더에 input.ply로 저장
            json_cams = []
            camlist = []
            if scene_info.test_cameras: #none
                camlist.extend(scene_info.test_cameras)
            if scene_info.train_cameras: #uid, R(3x3),T(3X1), FovY, FovX, path, w,h etc..
        ...
        if self.loaded_iter:
            if opt.expand_train and (opt.get_video or opt.eval):
                self.gaussians.load_ply(os.path.join(self.model_path,
                                            "point_cloud",
                                            "iteration_" + str(self.loaded_iter),
                                            "point_cloud_PIGS.ply"))
            else:
                self.gaussians.load_ply(os.path.join(self.model_path,
                                "point_cloud",
                                "iteration_" + str(self.loaded_iter),
    ...
    def save(self, iteration, expand_train):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        if expand_train:
            self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud_PIGS.ply"))
        else:
